...
class Button(pygame.sprite.Sprite):
    ...
    def loop(
        ...
        self.on = (
            self.on[1],
            pygame.Rect.collidepoint(self.rects[2], mouse_pos[1]),
        )
        if self.on[1]:
        ...
        if (
            ...
            and self.check
            and self.on[0]
            or (self.check and not self.on[0])
        ):
            self.check = False
            self.released = 1
...
class Slider(Button):
    ...
    def value(self, value):
        ...
        value = int(
            pygame.math.clamp(
                value
                - mod
                + (self.stepSize if mod * 2 >= self.stepSize else 0),
                self.valueMap[0],
                self.valueMap[1],
            )
        )
        ...
        self._value = value
        self.pos[not self.horizontal] = (
            (value - self.valueMap[0]) / (self.valueMap[1] - self.valueMap[0])
        ) * (self.posMap[1] - self.posMap[0]) + self.posMap[0]
    ...
    def loop(
        ...
        if self.grabbed and not self.clicked:
            self.value = (
                (mouse_pos[1][not self.horizontal] - self.posMap[0])
                / (self.posMap[1] - self.posMap[0])
            ) * (self.valueMap[1] - self.valueMap[0]) + self.valueMap[0]
