...
from multi_sprite_renderer_hardware import (
    MultiSprite as Msr,
    rotated_collision,
)
...
class Monster:
    ...
    def __init__(self, sprites: Msr, scale):
        ...
        self.leftLeg = Limb(
            self.msr, self.pos, self.speed, left=-1, scale=self.scale
        )
        self.rightLeg = Limb(
            self.msr, self.pos, self.speed, left=1, scale=self.scale
        )
    ...
    def update(self, dt):
        ...
        if Sm.app.mobile:
            if (
                self.pos.distance_squared_to(Button.mousePos[1])
                > (192 / 2.5 * 0.6875) ** 2
            ):
        ...
        if self.health > 0:
            ...
            self.health += (
                4.5
                + (self.game.wave / 2)
                * (len(self.game.astros) / self.game.astrosLimit)
            ) * dt
        ...
        elif self.wallRect.collidepoint(
            self.pos + direction * self.speed * dt * 10
        ):
            pos += direction * self.speed * dt * 10
            arm_pos += direction * self.speed * dt * 10
            moved = direction
        elif self.wallRect.collidepoint(
            self.pos + direction2 * self.speed * dt * 10
        ):
        ...
        # arm calculation
        if (
            relativeOffset := self.tentacle.endPos - mouse_pos_vect
        ).length_squared() > (self.tentacle.length * 2 + 4) ** 2:
            self.armPos.move_towards_ip(
                self.tentacle.endPos
                + relativeOffset.normalize() * -(self.tentacle.length * 2 + 4),
                self.armSpeed * dt * 10,
            )
        else:
            self.armPos.move_towards_ip(
                mouse_pos_vect, self.armSpeed * dt * 10
            )
    ...
    def body_draw(self):
        ...
        if self.blink <= 0.06 or self.health <= 0:
            self.msr.draw(
                3,
                scale=(2 * self.scale, 2 * self.scale),
                pos=self.pos,
                relativeOffset=(0, 0),
            )
            if self.blink <= 0:
        ...
        self.msr.draw(
            4,
            scale=(self.scale / 0.6875, 1.5 * self.scale),
            pos=self.pos,
            relativeOffset=(0, 2.5),
        )
        self.msr.draw(
            5,
            scale=(
                max(self.health / 100, 0) * self.scale / 0.6875,
                1.5 * self.scale,
            ),
            pos=self.pos + (-16 * self.scale / 0.6875, 0),
            relativeOffset=(-0.5, 2.5),
            offset=(-0.4, 0),
        )
    ...
    def collision(self, hits):
        if hits:
            for item in hits:
                if isinstance(item, Laser) and rotated_collision(
                    (self.rect, 0), item.rects
                ):
                    if self.health > 0:
                        self.hitsTaken += 1
...
class Limb:
    ...
    def __init__(self, sprites, body_pos, body_speed, left=-1, scale=1.0):
        ...
        self.end = self.bodyPos + (
            (self.upper * 1.2 + 15 * self.scale) * self.left,
            self.lower * 0.6,
        )  # ending of the leg
    ...
    def update(self, dt, move, step):
        ...
        # decide to step
        if (
            limb_dist > self.maxDist
            or (self.end - self.bodyPos).length() > self.maxDist * 4
            or (self.end.y - self.bodyPos.y) < self.maxDist * 0.4
        ) and step:
            self.next = limb_pos + (
                move.x * self.bodySpeed * 2,
                move.y * self.bodySpeed * 2,
            )
        ...
        if self.prev.x - self.next.x:
            self.end.y = (
                math.sin(
                    abs(self.end.x - self.next.x)
                    / abs(self.prev.x - self.next.x)
                    * math.pi
                )
                * -0.06
                * abs(self.prev.x - self.next.x)
    ...
    def draw(self):
        ...
        self.msr.draw(
            name=2,
            scale=[self.upper / 16, 0.9 * self.scale],
            pos=limb[0],
        ...
        self.msr.draw(
            name=2,
            scale=[self.lower / 16, 0.8 * self.scale],
            pos=limb[2],
            relativeOffset=(-0.5, 0),
            rotation=limb[3],
        )
        ...
        self.msr.draw(
            name=3,
            scale=[0.9 * self.scale, 0.9 * self.scale],
            pos=limb[2],
            relativeOffset=(0, 0),
            rotation=limb[3],
        )
        self.msr.draw(
            name=3,
            scale=[1.2 * self.scale, 0.9 * self.scale],
            pos=limb[4],
            relativeOffset=(0.05, 0),
            rotation=0,
        )
    ...
    def limb(
        ...
        """
        returns: start pos, first rotation, joint pos,
        second rotation, real end pos, distance from targeted end pos
        """
        ...
        dist = pygame.math.clamp(
            dist, max(abs(lower - upper), min_length), lower + upper
        )
        aim = pygame.Vector2.angle_to(aim, (1, 0)) % 360

        # source: https://i.ytimg.com/vi/IKOGwoJ2HLk/maxresdefault.jpg
        u_rot = math.acos(
            (dist**2 - lower**2 - upper**2) / (2 * lower * upper)
        )
        l_rot = math.atan(
            (upper * math.sin(u_rot)) / (lower + upper * math.cos(u_rot))
        )
...
class Tentacle:
    ...
    def __init__(
        self, sprites, end, length, rot, links, start=None, scale=1.0
    ):
        self.msr = sprites
        self.scale = scale
        self.timer = 0
    ...
    def update(self, dt, end, start=None):
        ...
        for k, (link, _) in enumerate(self.links):
            ...
            rot += (
                math.sin(k / self.count * 16 + self.timer * 4) * 4 * dt * 45
            )  # wiggle
    ...
    def draw(self):
        for k, (link, rot) in enumerate(self.links):
            self.msr.draw(
                name=3,
                scale=[1.4 * self.scale, (0.9 + (k * 0.05)) * self.scale],
                pos=link,
                relativeOffset=(0.5, 0),
                rotation=rot,
                flip=[0, 0],
